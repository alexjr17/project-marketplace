...
import type { MfgWarehouse } from '../../types/manufacturing';

const empty = { name: '', code: '', address: '', isActive: true };

export default function WarehousesPage() {
  const [items, setItems] = useState<MfgWarehouse[]>([]);
  const [loading, setLoading] = useState(true);
  const [modal, setModal] = useState(false);
  const [editing, setEditing] = useState<MfgWarehouse | null>(null);
  const [form, setForm] = useState<Partial<MfgWarehouse>>(empty);
  const [saving, setSaving] = useState(false);

  const load = async () => {
    setLoading(true);
    try { setItems(await manufacturingService.getWarehouses()); }
    catch { toast.error('No se pudieron cargar las bodegas'); }
    finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);

  const openNew = () => { setEditing(null); setForm(empty); setModal(true); };
  const openEdit = (w: MfgWarehouse) => { setEditing(w); setForm({ ...w }); setModal(true); };

  const save = async () => {
    if (!form.name?.trim()) { toast.error('El nombre es obligatorio'); return; }
    setSaving(true);
    try {
      if (editing) { await manufacturingService.updateWarehouse(editing.id, form); toast.success('Bodega actualizada'); }
      else { await manufacturingService.createWarehouse(form); toast.success('Bodega creada'); }
      setModal(false); load();
    } catch { toast.error('No se pudo guardar'); }
    finally { setSaving(false); }
  };

  const remove = async (w: MfgWarehouse) => {
    if (!confirm(`¿Eliminar la bodega "${w.name}"?`)) return;
    try { await manufacturingService.deleteWarehouse(w.id); toast.success('Bodega eliminada'); load(); }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-orange-100 rounded-lg"><Warehouse className="w-6 h-6 text-orange-600" /></div>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Bodegas</h1>
            <p className="text-sm text-gray-500">Almacenes donde queda el producto terminado.</p>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">Sin bodegas. Crea la primera.</td></tr>
            ) : items.map((w) => (
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setModal(false)}>
          <div className="bg-white rounded-2xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-gray-900 mb-4">{editing ? 'Editar bodega' : 'Nueva bodega'}</h2>
            <div className="space-y-3">
              ...
              <label className="block">
                ...
                <input value={form.address ?? ''} onChange={(e) => setForm({ ...form, address: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2" />
              </label>
